...
const lib_1 = require("ikagaka-shell-loader/lib/");
...
QUnit.test('SR.SurfaceState', (assert) => __awaiter(this, void 0, void 0, function* () {
    ...
    const dic = yield cvt(dir.new("shell/master"));
    const shell = yield lib_1.load(dic);
    console.log(shell);
    // 当たり判定表示
...
QUnit.test('Renderer', (assert) => __awaiter(this, void 0, void 0, function* () {
    ...
    const shellState = new SHS.ShellState(shell);
    // ベースサーフェス生成器
    const baseCache = new Renderer_1.SurfaceBaseRenderer(shell);
    // プリロードすると安心だけど重い
    ...
    const realCanvas = Renderer_1.Util.createCanvas(width, height);
    document.body.appendChild(realCanvas);
    // レンダラに実 DOM canvas を attach
    const rndr = new Renderer_1.SurfacePatternRenderer(baseCache);
    rndr.attachCanvas(new Renderer_1.Canvas(realCanvas));
